...
const Footer: React.FC<{ data: FooterData }> = ({ data }) => {
  const { logo, tagline, email, phone, address, copyright, links, social } =
    data;
  return (
    <footer
      id="contact"
      className="relative border-t border-slate-900/[.07] bg-[#0a1a33]"
    >
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1.7fr_1fr] gap-16 py-16">
          ...
          <div>
            <div className="flex items-center gap-0.5 mb-4">
              <span className="font-display text-[1.4rem] font-black text-white tracking-tight">
                {logo}
              </span>
              <span className="w-1.5 h-1.5 rounded-full bg-[#4d9cff] mb-3.5 dot-pulse" />
            </div>
            <p className="text-slate-400 text-[.875rem] leading-relaxed max-w-xs mb-7">
              {tagline}
            </p>
          ...
          {/* Contact */}
          <div>
            <h4 className="text-[.66rem] font-black uppercase tracking-[.18em] text-slate-500 mb-6">
              Contact Us
            </h4>
                ...
                ({ href, icon, val }) =>
                  val && (
                    ...
                    >
                      {icon === "email" ? (
                        <svg
                          className="w-4 h-4 mt-0.5 shrink-0 text-slate-500 group-hover:text-[#4d9cff] transition-colors"
                          fill="none"
                          stroke="currentColor"
                          strokeWidth={1.7}
                          viewBox="0 0 24 24"
